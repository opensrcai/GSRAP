...
static Camera CreateCameras() {
  Camera camera = {};
  {
    // Set optical axis and upward direction in the world coordinate system.
    // Variable optional_axis_w is equal to the z axis of the camera coordinate
    // system.
    const Vec3d camera_optical_axis_w = Vec3d(-1.0, 1.0, 1.0).normalized();
    const Vec3d camera_up_w           = Vec3d(0.0, -1.0, 0.0);

    // Compute x and y axes of the camera coordinate system.
...
static void RunP3pSolver(void) {
  ...
  p3p_solver_policy.algorithm = gsrap::P3pSolverAlgorithm::KE_2017;
  ...
  pnp_inlier_check_params.inlier_thr                             = 1e-15;
